<?php
    if (isset($_GET["alterar"])):
        ...
        $sql = "SELECT id, nome, email, cidade, texto FROM tads.recados WHERE 
        id={$id};";
    
    if($result = $con->query($sql)):
        if($result->num_rows -> fetch_object('RecadoDAO'));
            endif;
        endif;
...
<form action="<?= isset($recado->id)?"?atualizar=$recado->id":""?>" method="post">
    ...
    Recado:
    <textarea name="recado" id="recado" cols="30" rows="10" value="<?= $recado->texto??''?>"></textarea><br>
...
<?php
    if(isset($_POST["enviar"])):
        ...
        elseif ($_POST["enviar"] == "Atualizar"):
            $id = $_GET["atualizar"];
            $sql = "UPDATE tads.recados SET nome= '{$nome}', email= '{$email}', cidade =
            '{$cidade}', texto='{$texto}' WHERE id={$id};";
